...
package com.mirza.mmusic.ACRCloudClasses;
...
public class ACRCloudRecognizer {
    ...
    public ACRCloudRecognizer(Map<String, Object> config) {
        ...
        if (config.get("debug") != null) {
            ...
            if (this.debug) {
                com.acrcloud.utils.ACRCloudExtrTool.setDebug();
            }
        }
    }
    ...
    public String recognize(byte[] wavAudioBuffer, int wavAudioBufferLen) {
        byte[] fp = com.acrcloud.utils.ACRCloudExtrTool.createFingerprint(wavAudioBuffer, wavAudioBufferLen, false);
        if (fp == null) {
    ...
    public String recognizeByFileBuffer(byte[] fileBuffer, int fileBufferLen, int startSeconds) {
        byte[] fp = com.acrcloud.utils.ACRCloudExtrTool.createFingerprintByFileBuffer(fileBuffer, fileBufferLen, startSeconds, 12, false);
        if (fp == null) {
            return ACRCloudStatusCode.DECODE_AUDIO_ERROR;
        }
        if (fp.length <= 0) {
    ...
    public String recognizeByFile(String filePath, int startSeconds) {
        byte[] fp = com.acrcloud.utils.ACRCloudExtrTool.createFingerprintByFile(filePath, startSeconds, 12, false);
